...
return val.toString();}},self=function(rawData){if(!rawData){return{};}
var
...
return newData;};self.cleaners=cleaners;return self;}());PLISTA.getItemData_clean=PLISTA.items.clean;PLISTA.items.getByObjectId=function(objectid,domainid){var i,item;if(!domainid){domainid=PLISTA.domainid;}
for(i=0;i<PLISTA.items.length;i++){item=PLISTA.items[i];if((item.objectid==objectid)&&(!item.domainid||domainid==item.domainid)){return item;}}};PLISTA.lib={};PLISTA.options={sendItemDataAttributes:[]};PLISTA.onBeforeInit=PLISTA.util.newEvent(PLISTA,'beforeinit');PLISTA.onLoad=PLISTA.util.newEvent(PLISTA,'load');PLISTA.onClick=PLISTA.util.newEvent(PLISTA,'click');PLISTA.onSendItemData=PLISTA.util.newEvent(PLISTA,'senditemdata');PLISTA.click=function(args,track){var i,name,request=[];if(arguments.length===1){track=true;}
if(track===true){for(name in args){if(args.hasOwnProperty(name)){switch(typeof args[name]){case'undefined':break;case'object':if(!args[name]){break;}
...
request.push('clientrev='+PLISTA.info.clientrev);if(document.images){(new Image()).src=PLISTA.path+'redirect.php?'+request.join('&');}}
PLISTA.onClick.fire(args);return true;};PLISTA.isArticle=function(){return(PLISTA.items.length===1);};PLISTA.currentObjectid=function(){if(PLISTA.items.length){return PLISTA.items[0].objectid;}};PLISTA.addItem=function(item,fn){PLISTA.remotedata('additem',{item:PLISTA.items.clean(item)},fn);PLISTA.onSendItemData.fire(item);};PLISTA.addCategory=function(item,cat){PLISTA.remotedata('assigncategory',{objectid:item.objectid,categoryname:cat});};PLISTA.loadWidgetData=(function(){var hasRun=false,logger=PLISTA.util.logger.getLogger('loadWidgetData');return function(){var isarticlerequest,attrs,k;if(hasRun){logger.debug('I did my work already. Aborting...');return;}
...
tag.type='text/javascript';tag.src=url;document.getElementsByTagName('head')[0].appendChild(tag);return tag;};PLISTA.loadModules=function(names,fn,force,cond){cond=cond||function(){return names.reduce(function(prev,val){return prev&&PLISTA[val];},true);};if(cond()&&!force){if(typeof fn==='function'){fn();}
return;}
...
k=setTimeout(fn,t);}};PLISTA.selfmonitoring.onErrorReport=PLISTA.util.newEvent(PLISTA.selfmonitoring,'errorreport');reset();}());
...
request=handler+':'+that.getSpecificationString(request);callbacks.push({request:encodeURIComponent(request),onSetData:onSetData,onAfterDataSet:onAfterDataSet});that.setMode(that.mode);};function checkWidgetdataTimeout(cbName){setTimeout(function(){if(typeof window[cbName]==='function'){PLISTA.addRemoteScriptTag(PLISTA.path+'errorreport.php?error=remotedata%20timeout&domainid='+PLISTA.domainid+'&msgkey=timeout_async_30s');}},30000);}
...
return;}
httpRequests[requestID].time=httpRequests[requestID].time||(new Date()).getTime();var req=httpRequests[requestID][requestSubID];if(typeof req.onSetData==='function'){req.onSetData(data,httpRequests[requestID].time);req.onSetData=undefined;}};that.setTime=function(requestID){var req,i;for(i=0;i<httpRequests[requestID].length;i++){req=httpRequests[requestID][i];if(req&&(typeof req.onAfterDataSet==='function')){req.onAfterDataSet();req.onAfterDataSet=undefined;}}};that.getSpecificationString=(function(){var l,r,strVal=function(o){r+=1;if(r>5){return'NESTED TOO DEEP';}
...
return style;},defaultStyleElements:{image:function(item,args){var
src=item.img,dim='';if((args[0]||item.type==='pet')&&src&&item.objectid){src=PLISTA.render.imageUrl(item,args[0],args[1],args[2]);}
...
return s;},text:function(item,args,ctx){var s=item.text,m=ctx.listwidget.spec.metaPlaceholder,i,tmp;if(m&&m.text&&args[0]){for(i=0;i<m.text.length;i+=1){tmp=m.text[i];if((item.status&tmp.flags)===tmp.flags){return ctx.listwidget.spec.styleElements[tmp.styleElement].call(this,item,tmp.params,ctx);}}}
return PLISTA.render.simpletext(s,args,ctx);},number:function(item,args,ctx){return ctx.position;},url:function(item){return((item.url.indexOf('%'))===-1)?encodeURI(item.url):item.url;},created_at:{format:'d.m.Y',getContent:function(item){var t=parseInt(item.created_at,10),format=this.format;if(!t){throw'created_at: item.created_at is not set or evaluates to 0';}
...
return val;}
var date=new Date(t*1000);[['%%','%'],['%Y',pad(date.getFullYear())],['%y',pad(date.getFullYear()%100)],['%m',pad(date.getMonth()+1)],['%d',pad(date.getDate())],['%H',pad(date.getHours())],['%i',pad(date.getMinutes())],['%s',pad(date.getSeconds())]].forEach(function(i){f=f.replace(i[0],i[1]);});return f;},i18n:{rating_description:{1:'irrelevant',2:'langweilig',3:'so lala',4:'gut',5:'exzellent'}},imageUrl:function(item,width,height,bgColor){if(item.status&parseInt('1024',10)){return item.img;}
var url=PLISTA.cdnpath+'image/resized/',hash=PLISTA.util.stringSum(item.img||'');return url+
(item.domainid||PLISTA.domainid)+'/'+
(bgColor?bgColor+'/':'')+
item.objectid+'_'+(width||0)+'x'+(height||0)+'_'+hash+((/\.gif$/.test(item.img))?'.gif':'.jpg');},resizeImage:function(img,usemargin){var pDim=PLISTA.util.DOM.getDimensions(img.parentNode),imgOrig=PLISTA.util.DOM.getDimensions(img),ratio={x:pDim.x/imgOrig.x,y:pDim.y/imgOrig.y},t;if(isNaN(pDim.x+pDim.y+imgOrig.x+imgOrig.y)){return false;}
...
that.refresh();},refresh:function(){var i,html='',data=spec.list.getData(),style,ctx=spec.context||{},tempCtx,couponCalls=[],refreshCallbacks=[];PLISTA.util.DOM.event.removeAll(eventkey);ctx.listwidget=that;if((typeof data==='object')&&data&&(typeof data.length==='number')){if(spec.petpos){data=construct.reorderItems(data,spec.petpos);}
if(spec.shuffle){data=construct.arrayShuffle(data);}
...
return false;},'listwidget_2989');}});});that.setList(spec.list);PLISTA.listwidgets.push(that);return that;};construct.arrayShuffle=function(a){var
tmp,rand,l=a.length,i;for(i=0;i<l;i+=1){rand=Math.floor(Math.random()*l);tmp=a[i];a[i]=a[rand];a[rand]=tmp;}
return a;};construct.reorderItems=function(arr,petpos){var osr=arr.filter(function(i){return!(i.status&2);}),pets=arr.filter(function(i){return i.status&2;}),result=[],i;if(petpos===-1){petpos=Math.floor(Math.random()*arr.length)+1;}
if(!Array.isArray(petpos)){petpos=[petpos];}
petpos.forEach(function(pos){if(pets.length){result[pos-1]=pets.shift();}});for(i=0;i<arr.length;i+=1){if(!result[i]&&osr.length){result[i]=osr.shift();}}
return result.concat(pets).filter(function(i){return i;});};construct.htmlSafeItemData=function(item){var safe={},name;for(name in item){if(item.hasOwnProperty(name)){if(typeof item[name]==='string'&&name!=='html'){safe[name]=PLISTA.util.htmlEntities(item[name]);}else{safe[name]=item[name];}}}
...
PLISTA.partner.options=d;});PLISTA.partner={options:null,widgets:[],placements:[],determineOverrideName:function(osettings){var n,key,rand=Math.random(),p=0,line,tmp;for(n in osettings){if(osettings.hasOwnProperty(n)){line=osettings[n];key=line[0].split(':');if(key[0]==='p'){p=parseFloat(key[1])+p;if(p>rand){return line[1];}else if(p>1){throw new Error('sum of all probabilities is larger than 1 o.O');}}else if(key[0]==='ua'){tmp=key[1].match(/^\/(.+)\/([igm])?/);if((new RegExp(tmp[1],tmp[2])).test(navigator.userAgent)){return line[1];}}}}},init:(function(){var hasRun=false;PLISTA.onReset.subscribe(function(){hasRun=false;});return function(loc,init){if(hasRun){throw new Error('PLISTA.partner.init must not be called twice in a row. If you need to re-initialize plista, please call PLISTA.reset first.');}
hasRun=true;if(PLISTA.isInited()){PLISTA.util.logger.getLogger('partner.init').error('did not run PLISTA.partner.init because PLISTA.init was called before');}
setTimeout(function(){var ws=[];try{PLISTA.partner.init.onBefore.fire();PLISTA.partner.placements.forEach(function(p){var w,target=false;if(loc&&loc[p.name]){target=loc[p.name];}else if(typeof p.target==='string'){target=document.getElementById(p.target);}
if(!target){return;}
w=PLISTA.partner.widgets[PLISTA.partner.determineOverrideName(p.override)||p.name]();w.target=target;ws.push(w);});PLISTA.partner.initWidgets(ws);if(init!==false){PLISTA.init();}}catch(e){PLISTA.util.logger.getLogger('partner_init').error(e);}},PLISTA.options.partnerInitDelay||0);};}()),initWidgets:function(widgets){if((typeof widgets!=='object')||(widgets===null)){return;}
...
widget.onRefresh.subscribe(addClickTracks,widget);if(document.location.hash==='#plista'){widget.onRefresh.subscribe(gotoFirstWidget);}
