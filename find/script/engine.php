...
namespace find\script;

/**
 * Description of blorkFunctions
 * @author christian, addOn to blork engine, juin 2020.
 */
function resultUrl($param) {
    echo $param;
}
...
/*
  include "../../app/common/JardinFonctions.php";

  $fn = new \app\common\JardinFonctions();
  $file = "../../culture/ombre.html";

  $fn->debug($file);

  $r = fopen("$file", "r");
  $fn->debug($r);
  $r = mb_strtolower(fread($r, 15000));
  $fn->debug($r, 'le contenu du fichier');
...
function erreur_creve($erreur_id, $raison, $engine_version) {
    ...
    echo("<font style=\"font-family:verdana; size:10pt;\">");
...
function form_recherche($contenu, $type) {
    ...
    echo(" <input type=\"text\" name=\"blork\" value=\"" . $contenu . "\" maxlength=\"50\" size=\"25\"> ");
    ...
    echo("value=\"exact\" name=\"type\"> expression exacte</form>");
}
    ...
    foreach ($d as $cle => $lien) {
        ...
        while ($file = readdir($fp)) {
            if ($file != '.' && $file != '..') {
                $verif = $lien . "/" . $file;
                if (
                        is_dir($verif) && !(in_array($verif, $d)) && !(in_array($verif, $exclu))) {
                    $d[] = $verif;
                }
            }
        }
...
function affiche_resultats(
    ...
    echo($nb_resultats . " résultats trouvés pour votre recherche sur <b>\"" . $blork .
    "\"</b>, dans " . $nb_fichiers . " " . $fich . ".
	$mot_supprime");
    ...
    if ($nb_resultats <= 0) {
        ...
        for ($i = $start * $maxi; $i < $start * $maxi + $maxi; $i++) {
            ...
            echo("<p>" . ($i + 1) . " - " . $tableau_resultats[$i] . "</p>");
        }
        ...
        if ($nb_resultats > $maxi) {
            ...
            if ($start >= 1) {
                echo("<A href=\"" . $page . "?blork=" . $blork . "&type=" . $type . "&start=" . ($start - 1) . "&cherche=go\"><< Résultats précédents</A>");
            }
            ...
            if ($_GET["start"] < $finstart) {
                echo("<A href=\"" . $page . "?blork=" . $blork . "&type=" . $type . "&start=" . ($start + 1) . "&cherche=go\">Résultats suivants >></A>");
            }
...
if (empty($config['maxipage']) || !is_numeric($config['maxipage'])
) {
...
if (empty($config['maxmots']) || !is_numeric($config['maxmots']) || $config['maxmots'] > 9
) {
...
//    "qui indique les dossiers à scanner (étape 2)" => "$dossier", // old, bug
...
if (!isset($_GET['cherche']) || !isset($_GET['blork']) || empty($_GET['cherche']) || empty($_GET['blork']) || $_GET['cherche'] != "go"
) {
...
if (!empty($_GET['type']) && !in_array($_GET['type'], $config['types_recherche'])
) {
    if ($config['debug_mode'] == "oui") {
        envoie_headers();
    } else {
...
// ereg_replace obsolète, squizzé
//$_GET['blork'] = ereg_replace(" +", " ", $_GET['blork']);

if (strlen($_GET['blork']) < 3) {
    echo("<br />Votre recherche doit comporter au moins trois caractères.<br />");
    $_GET['type'] = "exact";
...
if (!empty($_GET['type']) && $_GET['type'] == "exact") {
    ...
    if (
            count($recherche) <= 0 || strlen($_GET['blork']) < 3
    ) {
        $_GET['type'] = "";
        form_recherche($_GET['blork'], $_GET['type']);
        debug_footer_basta($config['debug_mode'], $chrono);
...
if (!empty($_SESSION['ze_blork']) && !empty($_SESSION['ze_type']) && $_SESSION['ze_blork'] == $_GET['blork'] && $_SESSION['ze_type'] == $_GET['type']) {
    // Formulaire du haut 
...
foreach ($recherche as $key => $value) {
    $recherche[$key] = mb_strtolower($value);
...
foreach ($dossier as $nomdos => $d) {
    ...
    while ($file = readdir($fp)) {
        ...
        // modifiée by C.A., le 02/06/2020 en :
        ...
        if (in_array($ext, $liste_extensions['txt'])) {
            ...
            $filetype = "img";
            $titre = mb_strtolower($file);
//            $titre = strtolower($file);
        } else {
            continue;
        }
//                            var_dump($titre);
//                            var_dump(utf8_encode($titre));
        ...
        if ($filetype == "texte") {
            ...
            // ou rb pour lire en binaire
            // $recupere_le_fichier = fopen("$d/$file", "rb");
//            $tout = file_get_contents("$d/$file",true, null, 0, 500000);
            $tout = mb_strtolower(fread($recupere_le_fichier, 500000));
//            $tout = strtolower(fread($recupere_le_fichier, 500000));
...
//            $tout = strtolower($tout);
             ...
             */
            /* le tag de fin n'est pas obligatoire, Cf doc PHP, C.C. le 03/06/2020
             * 
              $tout = strip_tags($tout, '<title></title><script></script><head></head><style></style><select></select>');
             * 
             */
            $tout = strip_tags($tout, '<title><script><head><style><select>');
        ...
        switch ($_GET['type']) {
            ...
            case "all":
                ...
                foreach ($recherche as $key => $value) {
                    if ($filetype == "texte" &&
                            (
                            strstr($tout, $value) || strstr(mb_strtolower($file), $value) || strstr(mb_strtolower($titre), $value)
//                            strstr($tout, $value) || strstr(strtolower($file), $value) || strstr(strtolower($titre), $value)
                            )) {
                        $mots_trouves++;
                    } elseif ($filetype == "img" && strstr(mb_strtolower($file), $value)) {
//                    } elseif ($filetype == "img" && strstr(strtolower($file), $value)) {
                ...
                if ($mots_trouves == count($recherche)) {
                    ...
                    // Source du résultat 
                    $src = " <A href=\"$go_2_url\">$titre ligne 1164 </A> <br /> $resume";
                    // On enregistre 
            ...
            case "exact":
                ...
                if ((!empty($tout) && strstr($tout, $recherche[0])) || strstr(mb_strtolower($file), $recherche[0]) || strstr(mb_strtolower($file), $recherche[0])
//                if ((!empty($tout) && strstr($tout, $recherche[0])) || strstr(strtolower($file), $recherche[0]) || strstr(strtolower($file), $recherche[0])
                ) {
...
print_r($d,'dossier, $d');
                    ...
                    // Source du résultat 
                    $src = " <A href=\"$go_2_url\">$titre ligne 1344</A> <br />$resume";
            ...
            case "now_exact":
                ...
                if ((!empty($tout) && strstr($tout, $recherche[0])) || strstr(mb_strtolower($file), $recherche[0]) || strstr(mb_strtolower($file), $recherche[0])
                    ...
                    // On remet les caractères spéciaux spécial URL pour éviter des bugs avec IE 
                    $file = rawurlencode($file);
                    print ("pos = " . intval(strpos($d, '../cu')) . "<br>" );
print_r("dossier =  $d<br>");
print_r("fichier = $file <br>");
//print_r("dossier = " . substr($d, 4,-1). '<br>');
//print_r("fichier = " . substr($file, 0). '<br>');
                    if ($go2url == "") {
                        $go_2_url = "$d/$file";
                    } else {
                        $go_2_url = "$go2url";
                    ...
                    // Source du résultat 
                    $src = " <A href=\"$go_2_url\">$titre ligne 1527</A> <br />$resume";
