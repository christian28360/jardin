...
function debug($var) {
    if (!is_string($var)) {
        ...
        echo "<br /><span style='background: black; color: cyan;'>Débug de " . $var . "</span>";
    }
}

// rempli un tableau avec le calendrier des semis dans un fichier XML
function AfficheCalendrierSemis($TypePlante) {
    // on charge le fichier
    $dom = new domDocument();
    $dom->load('culture/calendrier_semis.xml');
...
function LigneTete($TypePlante) {
    ...
    echo ( $lPlante == "Fleur" ) ? "<th class='FondNoir'>Type</th>" : "";  // colonne supplémentaire pour type de fleur
