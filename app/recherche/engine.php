...
include("blork_engine_haut.php");

// pour revenir sur la page de recherche (C.A.) :
$retourRecherche = $_SERVER['HTTP_REFERER'];

// on supprime tous les fichiers plus vieux d'un jour
// lecture du dossier
$handle = opendir('resultatRecherche');

if ($handle) {
    while (false !== ($entry = readdir($handle))) {
        if (!is_dir($entry)) {
            if (date_diff_now(filectime($dir . $entry)) > 1) {
//                echo "Le fichier : '<b>" . $entry . "'</b> a <b>" . date_diff_now(filectime($dir . $entry)) . "</b> jours<br>";
            }
        }
    }
    closedir($handle);
...
// ma fonction de calcul de l'âge du fichier résultat
...
// rajouté par mes soins car plantages :
$choix = "";
$action = isset($_GET['action']) ? $_GET['action'] : "";
$start = isset($_GET['start']) ? $_GET['start'] : 0;
$blork = trim(isset($_GET['blork']) ? $_GET['blork'] : 0);
//$blork = trim(isset($_GET['page']) ? $_GET['page'] : 0);
$multi = isset($_GET['multi']) ? $_GET['multi'] : "";

// fin de mes rajouts
// On adapte les variables selon la version de PHP
//$version_de_php = str_replace(".", "", phpversion());  // sans les séparateurs '.'
//var_dump($version_de_php);
/*
...
$form_recherche = "
    <form method=GET><br>Nouvelle recherche :<br>
        ...
        <input title='Tapez un mot, ou partie de mot (4 caractères minimum), pour une nouvelle recherche' 
            type=text 
            value=\"$blork2\" 
            minlength=4 
            maxlength=50 
...
echo "<title>Recherche de $blork2 </title>";
...
/*
  ...
  $chaine = "J'ai créé de la guitare, j'ai créé, de la RegEx.";
  //echo str_word_count($chaine);

  if (preg_match($find, $chaine)) {
  $pos = ( $casse == "i" ) ? stripos($chaine, $mot) : strpos($chaine, $mot);
  if ($pos === false) {
  echo "La chaîne '$mot' ne se trouve pas dans la chaîne '$chaine'";
  } else {
  echo "La chaîne '$mot' a été trouvée dans la chaîne '$chaine'";
  echo "<br />et débute à la position $pos";
  }
  echo 'trouvé à ' . $pos;
  } else {
  echo 'Non trouvé';
...
if ($action != "go") {
    echo("$form_recherche");
    include("blork_engine_bas.php");
    exit();
}

// On vérifie la longueur de la recherche
if ($longueur_blork <= 3) {
    echo("Votre recherche doit comporter plus de trois caractères.<br> $form_recherche");
    include("blork_engine_bas.php");
    exit();
}
...
// cette fonction PHP est mieux (C.A. le 05/12/2011)
        ...
        if (
//                $ext != "html" && $ext != "htm" && $ext != "php" && $ext != "php3" && $ext != "txt" && $ext != "php4"
                $ext != "html"
                //   && $ext != "gif"
                //   && $ext != "jpg"
                //   && $ext != "png"
...
// ne pourrait-on pas utiliser HTMLDECODE, à la place du foreach ?
...
// Maintenant que le fichier a été retraité (si nécessaire),
...
// les 2 lignes ci-dessous rajoutées par moi car bug :
        if (!isset($tout))
            $tout = "";
        if (!isset($titre))
            $titre = "";
...
//                print_r('totral mots => ' . $total_mots . "<br>");
// On crée la description
                ...
                $fin_position = intval($longueur_blork + 150);
...
// Enfin on ne garde que le chiffre des dizaines
            ...
            if ($go2url == "") {
                ...
                $go_2_url = str_replace('../..//', '', $go_2_url);
            }

//            print_r("retour = " . urlencode($retourRecherche). "<br>");
            // Source du résultat
            $src = "<a href='" . $go_2_url . "&retourRecherche='" . urlencode($retourRecherche) . ">" . $titre . "</a><br />" . $resume;
            // On enregistre
...
// Changement du texte selon les résultats entre singulier et pluriel
$rs = ( $compteresultats > 1 ) ? " résultats trouvés " : " résultat trouvé ";
...
if ($compteresultats > 0) {
    echo("<br> $compteresultats $rs sur $zetotal $fich - Affichage de $start à $pourvoir");
    echo("$form_recherche");

// on a des résultats, on va les charger
// ensuite on va essayer de les écrire dans un fichier html pour les afficher dans le corps du site
...
if ($compteresultats == 0) {
    echo("<br><b><span class='NoResult'>Votre recherche n'a donné aucun résultat. Essayez de l'élargir en y mettant moins de mots ou vérifiez son orthographe.</span></b>");
} else {
