...
if ($handle) {
    while (false !== ($entry = readdir($handle))) {
        if (!is_dir($entry)) {
            if (date_diff_now(fileatime($dir . $entry)) > 1){
                
            echo " fichier : '<b>" . $entry . "'</b>, nb jours : '<b>" . date_diff_now(fileatime($dir . $entry)) . "'</b> à supprimer<br>";
            }
        }
    }
    closedir($handle);
...
var_dump($now / 86400);
var_dump(fileatime($dir . "html.html") / 86400);
var_dump(ceil(($now / 86400) - (fileatime($dir . "html.html") / 86400)));

var_dump(getdate($time = time()));
var_dump($time);
exit;

print ("<br>");
echo " date création = " . fileatime($dir . "html.html") . "<br>";
echo " date création = " . fileatime("resultatRecherche/m95ibg4addpbm9icaaor8ipm13.html") . "<br>";
echo " date création = " . date("d/m/Y\TH:i:s\Z", fileatime($dir . "html.html")) . "<br>";
echo " date création = " . date("d/m/Y\TH:i:s\Z", getdate()['0']) / 86400 . "<br>";
...
<input title='coucou' type=text value=\"$blork2\" maxlength=50 size=25 name=page>
<input type=submit value='Aller, cherche ! !'><br>
</form>";
?>

<TITLE>Recherche de <?PHP echo($blork2); ?> - Powered By a Blork Engine
</TITLE>
<?php
$mot = "Cr��";
$entier = "[^a-zA-Z�]";
...
<?php
// On v�rifie que la recherche est correctement lancée
if ($action != "go") {
    echo("$form_recherche");
