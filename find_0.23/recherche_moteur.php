...
require("recherche_fonctions.php"); // quelques fonctions utilis�es
require("recherche_config.php");    // Param�tres, dossiers exclus ...

debug("on est dans recherche_moteur.php") ;
...
$blork  = ( isset( $_GET['blork'] ) )  ? $_GET['blork']  : "";
$start  = ( isset( $_GET['start'] ) )  ? $_GET['start']  : "";
$multi  = ( isset( $_GET['multi'] ) )  ? $_GET['multi']  : "";

// Variables par d�faut
/*$maxmots= ( ! isset( $maxmots ) || $maxmots == "" ) ? "20" : $maxmots;
...
if ( $action == "" )
{
	...
	debug ("affiche formulaire");

	echo("$form_recherche");
}
else
	// formulaire chag�, on lance la rechzerche
{
	debug ("recherche");
	$creer_fichier = false;  // si recherche infructueuse ==> pas de cr�ation
	// Init r�sultats � 0
	$compteresultats  = "0";
	$zetotal          = "0";


	// traite tous les fichiers depuis le r�pertoire dossier["mon site"]
	recherche_mot($dossier["mon site"]);
...
if ( $scan_sousdos == "on" ) {
	...
	foreach ( $dossier as $nomdos => $d ){
		...
		while ( $file = readdir($fp) ) {
			if ( $file != '.' && $file != '..') {
				$verif  =  $d . "/" . $file;
				if( is_dir( $verif ) && !( in_array( $verif, $dossier)) && !(in_array($verif, $exclu_fichier))){
					$dossier[] = "$verif";
					$encore1[] = "$verif";
				}
			}
